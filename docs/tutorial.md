...
| ttl | 保存推送的有效期，单位为秒。仅对保存到历史记录的消息生效，到期后会自动删除；如果通知中心里仍有对应通知，也会一并删除。 |
...
“发送推送到此设备” 快捷指令支持 “加密发送” 开关，开启后使用 App 内加密设置发送，仅支持 GCM 模式，未配置加密或配置的模式不是 GCM 时会报错。<br>
“发送推送到其他设备” 快捷指令支持 “加密方式” 选择（不加密 / 共享密钥，默认不加密）：选不加密时密钥字段必须留空——填了密钥却选不加密会报错，以防本想加密的推送被误发为明文；选共享密钥时必填 “加密密钥（GCM）”，16 / 24 / 32 位密钥分别对应 AES128 / AES192 / AES256，模式固定为 GCM，无需在 App 内预先配置加密。
